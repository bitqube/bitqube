...
The RPM spec file provided here is for BitQube-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bitqube.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BitQube git repository](https://github.com/BitQubeProject/BitQube) but are not part of
...
Currently only `Source10` is used, for the example `bitqube.conf` file.
...
in the bitqube git repository. At present only an SVG version of the BitQube
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bitqube
...
needed when 0.12.1 is released, a proper fix is already in the BitQube git
master branch.
...
idea to build BitQube Core against a static version of that library compiled
...
The `bitqube.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitqube-server` package. At this point, most RPM based
...
The `bitqube-server` package should have SELinux support. How to properly do
...
BitQube-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitqube/). He has been
packaging BitQube for Fedora at least since 2012.
...
4. I renamed the `bitqube` package that contains the Qt GUI to `bitqube-core` as
that appears to be how the general population refers to it, in contrast to
`bitqube-xt` or `bitqube-classic`. I wanted to make sure the general population
