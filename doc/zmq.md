...
The BitQube Core daemon can be configured to act as a trusted "border
router", implementing the bitqube wire protocol and relay, making
...
The ZeroMQ feature in BitQube Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bitqubed:
...
For instance:
    ...
    $ bitqubed -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/bitqubed.tx.raw
...
These options can also be provided in bitqube.conf.
...
From the perspective of bitqubed, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bitqubed directly. Furthermore, no information is
...
using. BitQubed appends an up-counting sequence number to each
