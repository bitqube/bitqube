...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for BTQ
    ...
    if (p_rewardSnapshot.strDistributionAsset != "BTQ") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is BTQ with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using BTQ differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "BTQ") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse BitQube address (already validated during ownership snapshot creation)
