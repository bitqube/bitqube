...
{
    const char* pszTimestamp = "Key Democratic lawmakers say crypto Clarity Act 'falls short' on ethics, other issues";
    const CScript genesisOutputScript = CScript() << ParseHex("04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38c4f35504e51ec112de5c384df7ba0b8d578a4c702b6bf11d5f") << OP_CHECKSIG;
    return CreateGenesisBlock(pszTimestamp, genesisOutputScript, nTime, nNonce, nBits, nVersion, genesisReward);
}

#ifdef MINE_GENESIS
// BitQube: one-shot genesis miner. Compile with -DMINE_GENESIS, run bitqubed for each
// network (mainnet / -testnet / -regtest); it prints the nonce, genesis hash and merkle
// root, then exits. Bake the printed values into the CreateGenesisBlock() calls and the
// asserts below, then rebuild WITHOUT -DMINE_GENESIS.
#include <cstdio>
#include <cstdlib>
static void MineGenesis(CBlock& genesis, uint32_t nBits, const char* net)
{
    arith_uint256 target;
    bool fNeg, fOver;
    target.SetCompact(nBits, &fNeg, &fOver);
    printf("\n[BitQube genesis miner] network=%s target=%s\n", net, target.GetHex().c_str());
    for (uint32_t nonce = 0; ; nonce++) {
        genesis.nNonce = nonce;
        uint256 hash = genesis.GetX16RHash();
        if (UintToArith256(hash) <= target) {
            printf("[BitQube genesis miner] FOUND net=%s\n", net);
            printf("    nTime  = %u\n", genesis.nTime);
            printf("    nBits  = 0x%08x\n", genesis.nBits);
            printf("    nonce  = %u\n", nonce);
            printf("    hash   = %s\n", hash.GetHex().c_str());
        ...
        if ((nonce & 0x3fffff) == 0) { printf("  ...net=%s nonce=%u\n", net, nonce); fflush(stdout); }
    }
}
#endif
...
public:
    CMainParams() {
        ...
        consensus.nCPUMiningEpochBlocks = 5000; // First 5,000 blocks pinned to powLimit (CPU-mineable launch)
        ...
        // The best chain should have at least this much work (fresh BitQube chain).
        consensus.nMinimumChainWork = uint256S("0x00");

        // By default assume that the signatures in ancestors of this block are valid (fresh chain).
        ...
        pchMessageStart[3] = 0x4d; // M (mainnet)
        nDefaultPort = 8851;
        nPruneAfterHeight = 100000;

        genesis = CreateGenesisBlock(1784752134, 150274, 0x1f00ffff, 4, 0);
        consensus.hashGenesisBlock = genesis.GetX16RHash();
#ifdef MINE_GENESIS
        MineGenesis(genesis, 0x1f00ffff, "main");
#endif

        assert(consensus.hashGenesisBlock == uint256S("0x0000c9dc1810a24d2454bfa24aa07f0ff0288445f2fe64f2284f321b1063fccb"));
        assert(genesis.hashMerkleRoot == uint256S("0xfaa3fbf62298c6480f0975b612ddf7200d5e122f586952c6d9ae8c592b3ab716"));

        vSeeds.emplace_back("seed.bitqube.org", false);
        ...
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,153);  // WIF
        ...
        nTransferOverflowHeight = 0; // BitQube: asset transfer qty overflow checks enforced from genesis
        ...
        // BitQube: KAWPOW active from launch (genesis time). Genesis itself is X16R-mined;
        // every block after genesis uses KAWPOW.
        nKAAAWWWPOWActivationTime = 1784752135; // genesis time + 1s: genesis stays X16R-mined, every later block is KAWPOW
...
public:
    CTestNetParams() {
        ...
        // The best chain should have at least this much work (fresh BitQube testnet).
        consensus.nMinimumChainWork = uint256S("0x00");
        ...
        uint32_t nGenesisTime = 1784752134;  // 2026-07-22 20:28:54 UTC (BitQube genesis time)
...
//        /////////////////////////////////////////////////////////////////
        ...
        genesis = CreateGenesisBlock(nGenesisTime, 133152, 0x1f00ffff, 2, 0);
        consensus.hashGenesisBlock = genesis.GetX16RHash();
#ifdef MINE_GENESIS
        MineGenesis(genesis, 0x1f00ffff, "test");
#endif

        //Test MerkleRoot and GenesisBlock
        assert(consensus.hashGenesisBlock == uint256S("0x000052622ea82f1a64ea10bd5db7bbc4814ea7865f3d7ac22a9325215d57ae03"));
        assert(genesis.hashMerkleRoot == uint256S("0xfaa3fbf62298c6480f0975b612ddf7200d5e122f586952c6d9ae8c592b3ab716"));

        vFixedSeeds.clear();
        vSeeds.clear();

        vSeeds.emplace_back("seed-testnet.bitqube.org", false);
        ...
        // BitQube BIP44 cointype in testnet
        nExtCoinType = 1;

        // BitQube: bootstrap via the DNS seed (seed-testnet.bitqube.org). No stale fixed seeds.
        vFixedSeeds.clear();
        ...
        checkpointData = (CCheckpointData) {
            {
                // Fresh BitQube testnet: checkpoints to be added as the chain grows.
            }
        };

        chainTxData = ChainTxData{
            // Fresh BitQube testnet: no transaction history yet.
        ...
        nTransferOverflowHeight = 0; // testnet: overflow checks from genesis
        nMessagingActivationBlock = 1; // Messaging active from launch
        nRestrictedActivationBlock = 1; // Restricted assets active from launch
...
public:
    CRegTestParams() {
        ...
        consensus.nCPUMiningEpochBlocks = 0; // No CPU-mining epoch pin for regtest
        consensus.vDeployments[Consensus::DEPLOYMENT_TESTDUMMY].bit = 28;
...
//        /////////////////////////////////////////////////////////////////
        ...
        genesis = CreateGenesisBlock(1784752134, 0, 0x207fffff, 4, 0);
        consensus.hashGenesisBlock = genesis.GetX16RHash();
#ifdef MINE_GENESIS
        MineGenesis(genesis, 0x207fffff, "regtest");
#endif

        assert(consensus.hashGenesisBlock == uint256S("0x01b98cac4971b7e97b1f7dc42adc21515f1260c6f86962a4e2347e275cb2490d"));
        assert(genesis.hashMerkleRoot == uint256S("0xfaa3fbf62298c6480f0975b612ddf7200d5e122f586952c6d9ae8c592b3ab716"));
        ...
        // BitQube BIP44 cointype in regtest
        nExtCoinType = 1;
        ...
        nTransferOverflowHeight = 0; // regtest: overflow checks from genesis
