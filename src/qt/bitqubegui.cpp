...
#include "config/bitqube-config.h"
#endif

#include "bitqubegui.h"
...
const std::string BitQubeGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString BitQubeGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "BTQBTC"  , 1,          8},
    { "mBTC",   "BTQBTC"  , 1000,       5},
    { "µBTC",   "BTQBTC"  , 1000000,    2},
    { "Satoshi","BTQBTC"  , 100000000,  0},
    { "USDT",   "BTQUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(BitQubeGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

BitQubeGUI::BitQubeGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void BitQubeGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BitQube address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bitqube: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to BTQ addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BitQube addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BitQube addresses"));
    ...
    openAction->setStatusTip(tr("Open a bitqube: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BitQube command-line options").arg(tr(PACKAGE_NAME)));
...
void BitQubeGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* bitqubeLabelLayout = new QVBoxLayout(toolbarWidget);
        bitqubeLabelLayout->addWidget(labelToolbar);
        bitqubeLabelLayout->addWidget(m_toolbar);
        bitqubeLabelLayout->setDirection(QBoxLayout::TopToBottom);
        bitqubeLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("BitQube Market Price"));
        ...
        // BitQube: market-price display removed. Widgets stay constructed but hidden
        // (they are parented to headerWidget, so they must be explicitly hidden).
        ...
        // BitQube: market-price polling disabled — no external price requests are made.
        // (pricingTimer is left unstarted and getPriceInfo() is not called.)
        /** BTQ END */

        // Get the latest BitQube release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void BitQubeGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
...
void BitQubeGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool BitQubeGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool BitQubeGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BitQubeGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitQubeGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitQubeGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void BitQubeGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BitQubeGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BitQubeGUI::optionsClicked()
{
...
void BitQubeGUI::showDebugWindow()
{
...
void BitQubeGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BitQubeGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void BitQubeGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void BitQubeGUI::openClicked()
{
...
void BitQubeGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitQubeGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitQubeGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitQubeGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitQubeGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitQubeGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** BTQ START */
void BitQubeGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void BitQubeGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void BitQubeGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void BitQubeGUI::gotoRestrictedAssetsPage()
{
...
void BitQubeGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to BitQube network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void BitQubeGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitQubeGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void BitQubeGUI::updateHeadersSyncProgressLabel()
{
...
void BitQubeGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void BitQubeGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BitQube"); // default title
...
void BitQubeGUI::changeEvent(QEvent *e)
{
...
void BitQubeGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitQubeGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitQubeGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "BTQ")
        msg += tr("Amount: %1\n").arg(BitQubeUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(BitQubeUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void BitQubeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitQubeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitQubeGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitQubeGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitQubeGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void BitQubeGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitQubeGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitQubeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitQubeGUI::detectShutdown()
{
...
void BitQubeGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BitQubeGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void BitQubeGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(BitQubeGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void BitQubeGUI::subscribeToCoreSignals()
{
...
void BitQubeGUI::unsubscribeFromCoreSignals()
{
...
void BitQubeGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void BitQubeGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<BitQubeUnits::Unit> units = BitQubeUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitQubeUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(BitQubeUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(BitQubeUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (BitQubeUnits::Unit u : BitQubeUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(BitQubeUnits::name(u)), this);
...
{
    setText(BitQubeUnits::name(newUnits));
}
...
void BitQubeGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitQubeGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "BitQubeGUI::onPriceUnitChange: " + QString::number(newIndex);
