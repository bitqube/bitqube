...
#include "bitqubeaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitQube address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new BitQubeAddressEntryValidator(parent));
    widget->setCheckValidator(new BitQubeAddressCheckValidator(parent));
}
...
bool parseBitQubeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitqube: URI
    if(!uri.isValid() || uri.scheme() != QString("bitqube"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitQubeUnits::parse(BitQubeUnits::BTQ, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBitQubeURI(QString uri, SendCoinsRecipient *out)
{
    // Convert bitqube:// to bitqube:
    //
    //    Cannot handle this later, because bitqube:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("bitqube://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "bitqube:");
    }
    QUrl uriInstance(uri);
    return parseBitQubeURI(uriInstance, out);
}

QString formatBitQubeURI(const SendCoinsRecipient &info)
{
    QString ret = QString("bitqube:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitQubeUnits::format(BitQubeUnits::BTQ, info.amount, false, BitQubeUnits::separatorNever));
        paramCount++;
    }
...
bool openBitQubeConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(BITQUBE_CONF_FILENAME);
    ...
    /* Open bitqube.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitQube.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitQube (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("BitQube (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for BitQube*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitqube.desktop";
    return GetAutostartDir() / strprintf("bitqube-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a bitqube.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitQube\n";
        else
            optionFile << strprintf("Name=BitQube (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the bitqube app
...
bool GetStartOnSystemStartup()
{
    CFURLRef bitqubeAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (bitqubeAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, bitqubeAppUrl);

    CFRelease(bitqubeAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add bitqube app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, bitqubeAppUrl, nullptr, nullptr);
    }
