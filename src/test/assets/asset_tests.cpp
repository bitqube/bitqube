...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of BITQUBE NOT allowed
        BOOST_CHECK(!IsAssetNameValid("BTQ", type));
        BOOST_CHECK(!IsAssetNameValid("BITQUBE", type));
        BOOST_CHECK(!IsAssetNameValid("BITQUBE", type));

        //- Versions of BITQUBE ALLOWED
        BOOST_CHECK(IsAssetNameValid("BITQUBE.COIN", type));
        BOOST_CHECK(IsAssetNameValid("BITQUBE_COIN", type));
        BOOST_CHECK(IsAssetNameValid("BTQSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERBTQ", type));
        BOOST_CHECK(IsAssetNameValid("BITQUBESPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEBITQUBE", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_BITQUBES", type));
        BOOST_CHECK(IsAssetNameValid("SEBTQOT", type));
