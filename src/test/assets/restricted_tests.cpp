...
BOOST_FIXTURE_TEST_SUITE(restricted_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(verify_new_restricted_transaction_test) {
        ...
        CScript rvnTransfer = GetScriptForDestination(DecodeDestination(GetParams().GlobalFeeAddress()));
        CTxOut rvnOut(1*COIN, rvnTransfer);

        // Create transaction and add burn to it
        CScript burnScript = GetScriptForDestination(DecodeDestination(GetFeeAddress(AssetType::RESTRICTED)));
        ...
        CScript newRestrictedAssetOwnerScript = GetScriptForDestination(DecodeDestination(GetParams().GlobalFeeAddress()));
