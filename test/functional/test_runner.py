...
`test/functional/test_framework/test_framework.py:BitQubeTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/bitqube_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and bitqubed
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_bitqubed = config["components"].getboolean("ENABLE_BITQUBED")
    if not (enable_wallet and enable_cli and enable_bitqubed):
        print("No functional tests to run. Wallet, utils, and bitqubed must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if bitqubed is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "bitqubed"]) is not None:
            print("%sWARNING!%s There is already a bitqubed process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "BITQUBED" not in os.environ:
        os.environ["BITQUBED"] = build_dir + '/src/bitqubed' + exeext
        os.environ["BITQUBECLI"] = build_dir + '/src/bitqube-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `bitqube-cli help` (`rpc_interface.txt`).
